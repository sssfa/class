...
//a combination of: does a solution exist, and if so, what is it?u
std::pair<bool,std::pair<double,double>> quadratic(double a,double b,double c)
{
	...
	//otherwise get roots and return
		...
		return {true,{(-b+sqrt(radical))/2*a,(-b-sqrt(radical))/2*a}};
}
...
int main(){
	...
	if(res.first)
		cout<<"root1 is "<<roots.first<<",root2 is "<<roots.second<<endl;
